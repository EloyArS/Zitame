...
const createAppointment = async (date_time, userId, customerId, serviceId) => {
  const result = await pool.query(
    `INSERT INTO appointments (date_time, user_id, customer_id, service_id) 
     VALUES ($1, $2, $3, $4) 
     RETURNING *, date_time AT TIME ZONE 'Europe/Madrid' AS fecha_local`,
...
const getAppointmentsByUserId = async (userId) => {
  const result = await pool.query(
    `SELECT appointments.*, customers.name AS customer_name, customers.phone, services.name AS service_name, date_time AT TIME ZONE 'Europe/Madrid' AS fecha_local
         FROM appointments 
